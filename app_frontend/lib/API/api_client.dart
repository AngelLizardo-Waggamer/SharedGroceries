...
class ApiClient {
	...
	Future<Response> familiesRequest(FamiliesRoutes route, DTO data) {
		try {
			switch (route) {
				case .get:
					return _dio.get('families/$_apiVersion/user-family', data: null); // The endpoint does not require any data
				case .create:
  ...
  Future<Response> listsRequest(ListsRoutes route, DTO data) {
    try {
      switch (route) {
        case .create:
          return _dio.post('shopping-lists/$_apiVersion/create', data: data.toJson());
        case .list:
          return _dio.get('shopping-lists/$_apiVersion/list', data: null);
        case .listAll:
          return _dio.get('shopping-lists/$_apiVersion/list', queryParameters: {"includeInactive": true});
        case .delete:
          // Extract listId from DTO and embed it in the URL path
          final dto = data as ShoppingListIdDTO;
          return _dio.delete('shopping-lists/$_apiVersion/${dto.listId}', data: null);
        case .restore:
          // Extract listId from DTO and embed it in the URL path
          final dto = data as ShoppingListIdDTO;
          return _dio.post('shopping-lists/$_apiVersion/${dto.listId}/restore', data: null);
        case .status:
          ...
          return _dio.patch('shopping-lists/$_apiVersion/${dto.listId}/status', queryParameters: {"isActive": dto.isActive}, data: null);
      }
    } on DioException catch (e) {
      throw _handleError(e);
    }
